...
import com.lagu.platform.workflow.domain.OutboxEvent;
import com.lagu.platform.workflow.domain.OutboxEventRepository;
...
/**
 * Delivers committed workflow_outbox rows to Kafka, in creation order, at-least-once.
 * Same semantics as record-service's relay: broker failures stop the batch (retried next
 * tick, so later events can't overtake a failed one); unreadable poison rows are parked;
 * published rows are purged after a retention window.
 */
@Component

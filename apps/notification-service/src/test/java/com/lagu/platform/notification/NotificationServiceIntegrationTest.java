...
class NotificationServiceIntegrationTest {
    ...
    static void configure(DynamicPropertyRegistry r) {
        ...
        // None of these are actually used — EmailDeliveryService is @MockitoBean-replaced below
        // — but MailSenderAutoConfiguration's own condition evaluation resolves these
        // placeholders eagerly at context startup regardless, with no defaults in application.yml.
        r.add("spring.mail.host",     () -> "localhost");
        r.add("spring.mail.port",     () -> "1025");
        r.add("spring.mail.username", () -> "test");
        r.add("spring.mail.password", () -> "test");
        r.add("platform.notification.email.from", () -> "noreply@lagu.platform");
    }
    ...
    void automationEvent_inApp_createsNotification() {
        // Unique title: every test in this class shares one Testcontainers DB with no cleanup
        // between methods, and buildEvent's default title would otherwise collide with rows
        // other IN_APP-channel tests (markRead, markAllRead, ...) create and then mark read for
        // the same RECIPIENT_ID — a plain "first match" lookup could pick up one of those.
        String title = "Test Notification " + UUID.randomUUID();
        AutomationEvent event = buildEvent("IN_APP", RECIPIENT_ID, null, title);
    ...
    @Test
    void redeliveredEvent_doesNotCreateASecondNotificationOrResendEmail() {
        // Regression coverage for the review's idempotency finding: publishing the exact same
        // AutomationEvent (same eventId, as a real Kafka redelivery would) must not double the
        // in-app row or re-send the email.
        UUID eventId = UUID.randomUUID();
        String recipientEmail = "redelivered@example.com";
        AutomationEvent event = AutomationEvent.builder()
                .eventId(eventId)
                .eventType("ACTION_SUCCEEDED")
                .actionType("SEND_NOTIFICATION")
        ...
        kafkaTemplate.send(PlatformTopics.AUTOMATION_EVENTS, ORG_STR, event);
        kafkaTemplate.send(PlatformTopics.AUTOMATION_EVENTS, ORG_STR, event); // simulated redelivery

        Awaitility.await()
                .atMost(10, TimeUnit.SECONDS)
                .pollInterval(300, TimeUnit.MILLISECONDS)
                .untilAsserted(() -> {
                    var matching = notificationRepository.findAll().stream()
                            .filter(n -> "Redelivery Test".equals(n.getTitle()))
                            .toList();
                    assertThat(matching).hasSize(1);
                });

        // Give the (already-processed) second delivery a moment to be handled, then confirm the
        // email was still only sent once.
        Awaitility.await().during(2, TimeUnit.SECONDS).atMost(4, TimeUnit.SECONDS)
                .untilAsserted(() -> verify(emailDeliveryService, org.mockito.Mockito.times(1))
                        .send(recipientEmail, "Redelivery Test", "Should only ever be stored once"));
    }
    ...
    private AutomationEvent buildEvent(String channel, UUID recipientId, String recipientEmail) {
        return buildEvent(channel, recipientId, recipientEmail, "Test Notification");
    }

    private AutomationEvent buildEvent(String channel, UUID recipientId, String recipientEmail, String title) {
